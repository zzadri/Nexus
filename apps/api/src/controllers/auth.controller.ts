...
export class AuthController {
    static async register(req: FastifyRequest, res: FastifyReply) {
        try {
            const { email, password, displayName, csrfToken } =
                RegisterBodyZ.parse(req.body);
            ...
            if ("conflict" in out)
                return res.code(409).send({ error: out.conflict });

            const accessToken = await AuthService.startSession(
                (req as any).server,
                res,
                out.user.id,
                req
    ...
    static async loginTotp(req: FastifyRequest, res: FastifyReply) {
        try {
            ...
            if (!payload)
                return res.code(401).send({ error: "MFA_TOKEN_INVALID" });

            const user = await AuthService.getMe(payload.sub);
            if (!user || !user.twoFAEnabled)
                return res.code(401).send({ error: "MFA_NOT_SETUP" });

            const u = await (req as any).server.prisma.user.findUnique({
    ...
    static async refresh(req: FastifyRequest, res: FastifyReply) {
        const out = await AuthService.rotateRefresh(
            (req as any).server,
            res,
            req
        );
    ...
    static async twoFaSetup(req: any, res: FastifyReply) {
        const user = await (req as any).server.prisma.user.findUnique({
    ...
    static async twoFaVerify(req: any, res: FastifyReply) {
        ...
        if (!user?.twoFASecretEnc)
            return res.code(400).send({ error: "SETUP_FIRST" });
        if (!AuthService.verifyTotp(user.twoFASecretEnc, code))
            return res.code(401).send({ error: "TOTP_INVALID" });
